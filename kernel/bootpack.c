...
/*
 * 三把信号量：
 * 1) g_sem_counter_lock 保护“安全???数器”共??变量
 * 2) g_sem_rw_count      保护读者???数 g_rw_read_count
 * 3) g_sem_rw_resource   保护读写共享资源（写互斥、??????者加锁）
 */
...
/*
 * 监???窗口统计变量（volatile用于提示编译器这些值会??并发任务异???修改）
 * g_race_unsafe_* : 无锁竞争??
 * g_race_safe_*   : 信号量保护组
 * g_rw_*          : 读??-写者组
 */
...
/*
 * 无锁竞争任务：故意执行“???-??-写”非原子序列
 * 多任务并发时会出?? lost update（attempts > value??
 */
...
/*
 * 加锁竞争任务：同样的读改写，但由二值信号量保护临界??
 * 期望现象：attempts ?? value 基本一致，lost接近0
 */
...
struct SHEET *open_syncmon(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	/* 任一关键资源分配失败时直接返回，避免后续空指?? */
	if (sht == 0 || task == 0)
	{
		return 0;
	}

	/* 首???打开窗口时启动演示任务；后续重???打开??做监?? */
	syncdemo_start_once();
