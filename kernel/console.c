...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal)
{
	...
	{
		/*
		 * syncdemo: 鎵撳紑鈥滅珵浜夋潯浠?/淇″彿閲?/璇诲啓鑰呪�濈患鍚堢洃瑙嗙獥鍙?
		 * 璇存槑锛氶?栨?℃墽琛屼細瑙﹀彂鍐呮牳婕旂ず浠诲姟闆嗗垵濮嬪寲锛屽悗缁?鎵ц?屽彧鏂板?炵洃瑙嗙獥鍙?
		 */
...
void cmd_syncdemo(struct CONSOLE *cons, int memtotal)
{
	struct SHTCTL *shtctl = (struct SHTCTL *)*((int *)0x0fe4);
	/*
	 * 閫氳繃bootpack渚х獥鍙ｅ伐鍘傚嚱鏁版墦寮�鐩戣?嗙獥鍙ｃ�?
	 * open_syncmon杩斿洖0琛ㄧず璧勬簮涓嶈冻鎴栧垵濮嬪寲澶辫触銆?
	 */
